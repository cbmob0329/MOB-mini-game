...
(function(root){
  ...
  function program(pool,random=Math.random){
    ...
    return {
      qualifier:['monsterBoxMob',any(),pick(TAG,random),pick(['ohajikiMob','toyOnOff'],random),any(),any(),any(),any(),'deathGameChallenge','amidakujiMob'],
      repechage:[pick(['longJumpMob','bungeeMob'],random),pick(['cardShop','mobPinball'],random),'bowling3DMob'],
  ...
  function next(s,random=Math.random){
    ...
    return {key,multiplier:s.phase==='qualifier'&&[3,9].includes(s.round)?2:1,phase:s.phase,round:s.round+1,active:[...s.active]};
  }
  ...
  function submit(s,points,descriptor){
    ...
    for(const id of s.active){
      ...
      round[id]=team.members.reduce((sum,p)=>{const raw=Number(points[p]);if(!Number.isFinite(raw)||raw<0||raw>100)throw Error('Invalid participant score');individual[p]=raw*descriptor.multiplier;return sum+individual[p];},0);
    }
    ...
    const record={...descriptor,points:individual,teamPoints:round,totals:{...(['cutoff','championship'].includes(s.phase)?round:s.scores)},personal:{...(['cutoff','championship'].includes(s.phase)?individual:s.personal)},litBefore:beforeLit};s.history.push(record);s.round++;
  ...
  const api={TAG,program,create,next,ordered,submit};
