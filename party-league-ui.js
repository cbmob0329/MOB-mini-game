  ...
  window.MobPartyLeagueUI={create(api){
    ...
    function results({event,record},championShown=false){
      ...
      const rows=(total,round)=>teamRows(total).map((id,i)=>{
        ...
        return `<article class="league-rank ${qualified?'qualified':''} ${border?'league-border':''} ${record.phase==='final'&&(record.totals[id]||0)>=600?'league-lit':''}"><b>${teamRows(total).findIndex(t=>(total[t]||0)===pts)+1}</b><div>${portraits(id)}<strong>${team(id).name}${record.phase==='final'&&(record.totals[id]||0)>=600?' 💡 点灯':''}</strong><small>${team(id).members.map(p=>`${esc(member(p).name)} ${round?record.points[p]||0:record.personal[p]||0}pt`).join(' ＋ ')}</small>${tied?'<em>ボーダー同点・延長対象</em>':''}</div><strong>${pts}pt</strong>${border?`<footer>── ${cut}チーム通過ボーダー ──</footer>`:''}</article>`;
      }).join('');
      ...
      const show=()=>{const [title,body]=pages[page];draw(title,`<p>${labels[record.phase]} · GAME ${record.round} / ${api.title(record.key)}${record.multiplier===2?' / 2倍ポイント':''}</p>${page===1?`<p class="league-live">${record.phase==='final'?'600点で点灯。その次のゲーム以降の1位で優勝！':cut?`上位${cut}チームが通過！ ボーダー同点は延長戦。`:''}</p><aside class="league-cutline">${borderText}</aside>`:''}<div class="league-ranks">${body}</div><button id="leagueNext" class="primary league-continue">${page+1} / 2 · タップして次へ →</button>`);on('leagueNext',()=>{if(++page<pages.length){if(page===1)announce('現在の総合順位はこちら！','現在のチーム総合順位を発表！<br>今回の得点で順位はどう動いたのか。相棒と積み重ねた合計点に注目です！',[],show,'standings');else show();}else after(event);});};show();
    }
